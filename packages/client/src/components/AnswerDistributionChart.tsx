        ...
        <div className="ml-10">
          ...
          <div className="relative h-[200px] border-l-2 border-b-2 border-gray-500/50">
            ...
            <div className="absolute inset-0 flex items-end justify-around px-4">
              {stats.map((stat) => {
                const isCorrect = correctIndex !== null && stat.choiceIndex === correctIndex;
          ...
          <div className="flex justify-around px-4 mt-3">
            {stats.map((stat) => {
              const isCorrect = correctIndex !== null && stat.choiceIndex === correctIndex;
              const isPlayerAnswer = playerAnswer !== null && stat.choiceIndex === playerAnswer;
              return (
                <div
                  key={stat.choiceIndex}
      ...
      <div className="mt-6 pt-4 border-t border-millionaire-gold/30">
        <div className="flex justify-between items-center">
          <span className={`text-sm ${showCorrectAnswer ? 'text-emerald-400 font-semibold' : 'text-gray-300'}`}>
            Correct answers: {correctIndex !== null ? (stats[correctIndex]?.count || 0) : 0} ({correctIndex !== null ? (stats[correctIndex]?.percentage.toFixed(1) || 0) : 0}%)
          </span>
          <span className="text-sm text-gray-300">
            Incorrect: {totalResponses - (correctIndex !== null ? (stats[correctIndex]?.count || 0) : 0)}
          </span>
        </div>
      </div>
    </motion.div>
