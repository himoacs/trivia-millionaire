...
export function useSolace(config?: SolaceConfig) {
  ...
  useEffect(() => {
    ...
    const scheduleReconnect = () => {
      ...
      reconnectAttempt += 1;
      console.log(`🔄 [Client] Reconnecting in ${delay}ms (attempt ${reconnectAttempt})`);
    ...
    const createAndConnect = () => {
      ...
      session.on(solace.SessionEventCode.UP_NOTICE, () => {
        ...
        console.log('✅ [Client] Connected to Solace broker');
        reconnectAttempt = 0;
        setConnected(true);
      });
      ...
      session.on(solace.SessionEventCode.DISCONNECTED, () => {
        if (sessionRef.current !== session) return;
        console.log('🔌 [Client] Disconnected from Solace broker');
        setConnected(false);
        scheduleReconnect();
      });

      session.on(solace.SessionEventCode.SUBSCRIPTION_ERROR, (error) => {
        console.error('⚠️ [Client] Subscription error:', error);
      });

      session.on(solace.SessionEventCode.SUBSCRIPTION_OK, (event) => {
        console.log('📥 [Client] Subscription confirmed:', event.correlationKey);
      });
    ...
    const handleVisibility = () => {
      ...
      console.log('👁 [Client] Page visible — reconnecting now');
      reconnectAttempt = 0;
      createAndConnect();
    };
  ...
  const subscribe = useCallback((topicPattern: string, callback: MessageCallback) => {
    ...
    try {
      ...
      console.log(`📥 [Client] Subscribed to ${topicPattern}`);
  ...
  const publish = useCallback((topic: string, payload: any) => {
    ...
    try {
      ...
      console.log(`📤 [Client] Published to ${topic}:`, payload);
    } catch (error) {
